-- 010: Conversations RPC — lightweight DISTINCT ON + persons join.
-- prev_inbound is fetched separately via get_prev_inbound_batch to avoid
-- expensive lateral joins inside a single query on large message tables.
-- Removed SECURITY DEFINER to allow planner to inline the SQL function.
...
-- Batch fetch latest inbound message per person for outbound conversation previews.
-- Called separately from get_conversations to keep the main query fast.
DROP FUNCTION IF EXISTS public.get_prev_inbound_batch(text, uuid[]);

CREATE OR REPLACE FUNCTION public.get_prev_inbound_batch(p_user_id text, p_person_ids uuid[])
RETURNS TABLE (
  person_id uuid,
  body_text text,
  sender_name text
...
AS $$
  ...
  FROM messages m
  WHERE m.user_id = p_user_id
    AND m.person_id = ANY(p_person_ids)
