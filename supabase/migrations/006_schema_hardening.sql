-- Schema hardening: constraints, indexes, and safety fixes

-- connected_accounts: prevent duplicate rows per user+provider+account
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'connected_accounts_user_provider_account_key'
  ) THEN
...
CREATE INDEX IF NOT EXISTS connected_accounts_provider_account_idx
  ON connected_accounts(provider, account_id, status);

CREATE INDEX IF NOT EXISTS messages_thread_idx
  ON messages(user_id, thread_id, sent_at DESC);

CREATE INDEX IF NOT EXISTS messages_identity_idx
  ON messages(identity_id);

-- identities.person_id should not be null
ALTER TABLE identities ALTER COLUMN person_id SET NOT NULL;

-- Harden the backfill RPC against search_path manipulation
