...
2. **Polling fallback:** `useConversations` / `useThread` adjust `refetchInterval` to 15s when realtime is down, 60s when healthy. No in-hook `setInterval` — React Query owns the polling. This is the real reliability mechanism; realtime is an enhancement.

> Note: we do not enable `realtime.worker: true`. The worker is instantiated from a `blob:` URL, which is blocked by our Tauri CSP (`script-src 'self'`, no `worker-src`). Running realtime on the main thread is the default, battle-tested path and avoids a whole class of CSP failure modes.
